...
def main():
    ...
    viz = OptionsVisualizer(theme='plotly_dark')
    ...
    with tab1:
        ...
        try:
            ...
            pred_lower = current_spot - 150
            pred_upper = current_spot + 150
        
        st.markdown("---")
        
        # NEW: DIRECTIONAL SIGNALS SECTION
    ...
    with tab5:
        ...
        # Auto-suggest based on regime
        suggested_strategy, suggestion_reason = suggest_strategy(regime, pcr, current_vix, iv_skew)
        
        st.info(f"💡 **Suggested:** {suggested_strategy} - {suggestion_reason}")
        ...
        with col1:
            try:
                ...
                if strategy_type == 'long_call':
                    strike_offset = st.slider("Strike offset (points)", 0, 500, 100, 50)
                    call_strike = round(spot_price + strike_offset, -2)
                    premium_paid = st.number_input("Premium paid (₹)", value=150, step=50, min_value=10)
                    
                    strategy = StrategyTemplate("Long Call", spot_price, dte)
                    strategy.add_leg('CE', call_strike, 'buy', 1)
                    
                    # Payoff with premium ✅ NEW
                    entry_premiums = {('CE', call_strike, 'buy'): premium_paid}
                    
                    st.markdown(f"### {strategy.name}")
                    st.text(f"1. BUY {call_strike:.0f} CALL @ ₹{premium_paid}")
                    
                elif strategy_type == 'long_put':
                    strike_offset = st.slider("Strike offset (points)", -500, 0, -100, 50)
                    put_strike = round(spot_price + strike_offset, -2)
                    premium_paid = st.number_input("Premium paid (₹)", value=150, step=50, min_value=10)
                    
                    strategy = StrategyTemplate("Long Put", spot_price, dte)
                    strategy.add_leg('PE', put_strike, 'buy', 1)
                    
                    entry_premiums = {('PE', put_strike, 'buy'): premium_paid}
                    
                    st.markdown(f"### {strategy.name}")
                    st.text(f"1. BUY {put_strike:.0f} PUT @ ₹{premium_paid}")
                    
                elif strategy_type == 'iron_condor':
                    wing_width = st.slider("Wing Width", 100, 500, 200, 50)
                    strategy = IronCondor(spot_price, dte, wing_width=wing_width)
                    
                    # Premiums for IC
                    st.markdown(f"### {strategy.name}")
                    col_a, col_b, col_c, col_d = st.columns(4)
                    with col_a:
                        pe_buy_premium = st.number_input("PE Buy", value=30, step=10, min_value=5, key="pe_buy")
                    with col_b:
                        pe_sell_premium = st.number_input("PE Sell", value=80, step=10, min_value=5, key="pe_sell")
                    with col_c:
                        ce_sell_premium = st.number_input("CE Sell", value=80, step=10, min_value=5, key="ce_sell")
                    with col_d:
                        ce_buy_premium = st.number_input("CE Buy", value=30, step=10, min_value=5, key="ce_buy")
                    ...
                    entry_premiums = {
                        ('PE', strategy.legs[0]['strike'], 'buy'): pe_buy_premium,
                        ('PE', strategy.legs[1]['strike'], 'sell'): pe_sell_premium,
                        ('CE', strategy.legs[2]['strike'], 'sell'): ce_sell_premium,
                        ('CE', strategy.legs[3]['strike'], 'buy'): ce_buy_premium
                    }
                    
                elif strategy_type == 'strangle':
                    distance = st.slider("Strike Distance", 100, 500, 300, 50)
                    strategy = Strangle(spot_price, dte, delta_distance=distance)
                    
                    ce_premium = st.number_input("Call Premium (₹)", value=100, step=50, min_value=10, key="call_prem")
                    pe_premium = st.number_input("Put Premium (₹)", value=100, step=50, min_value=10, key="put_prem")
                    ...
                    entry_premiums = {
                        ('CE', strategy.legs[0]['strike'], 'buy'): ce_premium,
                        ('PE', strategy.legs[1]['strike'], 'buy'): pe_premium
                    }
                    
                elif strategy_type == 'straddle':
                    strategy = Straddle(spot_price, dte)
                    
                    premium = st.number_input("Premium per leg (₹)", value=150, step=50, min_value=10)
                    ...
                    entry_premiums = {
                        ('CE', strategy.legs[0]['strike'], 'buy'): premium,
                        ('PE', strategy.legs[1]['strike'], 'buy'): premium
                    }
                
                # Payoff profile WITH PREMIUM ✅ NOW CORRECT
                spot_range = np.linspace(spot_price * 0.90, spot_price * 1.10, 200)
                ...
                fig.add_vline(x=current_spot, line_dash="dash", line_color="blue", 
                             annotation_text=f"Current: {current_spot:.0f}")
                
                # Add breakevens if they exist
                breakevens = strategy.get_breakevens(premium=sum(entry_premiums.values()) if entry_premiums else 0)
                for be in breakevens:
                    fig.add_vline(x=be, line_dash="dot", line_color="orange", 
                                 annotation_text=f"BE: {be:.0f}")
                
                fig.update_layout(
                    title=f"{selected_strategy_name} - Payoff at Expiry (Premium Included ✅)",
                    xaxis_title="Nifty Level",
                ...
                st.session_state.entry_premiums = entry_premiums
            
            except Exception as e:
                st.error(f"Strategy builder: {e}")
                import traceback
        ...
        with col2:
            ...
            try:
                ...
                try:
                    ...
                    # Calculate portfolio Greeks for the strategy
                    greeks_inputs = []
                    for leg in strategy.legs:
                        greeks_inputs.append({
                            'spot': current_spot,
                            'strike': leg['strike'],
                            'time_to_expiry': 7/365,  # Using DTE from builder
                            'volatility': atm_iv / 100,  # Convert to decimal
                            'option_type': leg['type'],
                            'position': leg['position'],
                            'quantity': 1
                        })
                    
                    portfolio_greeks = calc.calculate_portfolio_greeks(greeks_inputs)
                    
                    with st.expander("📊 View Greeks", expanded=False):
                        g_col1, g_col2 = st.columns(2)
                        with g_col1:
                            st.metric("Delta", f"{portfolio_greeks['Delta']:.3f}")
                            st.metric("Gamma", f"{portfolio_greeks['Gamma']:.4f}")
                            st.metric("Vega", f"{portfolio_greeks['Vega']:.2f}")
                        with g_col2:
                            st.metric("Theta", f"{portfolio_greeks['Theta']:.2f}/day")
                            st.metric("Rho", f"{portfolio_greeks['Rho']:.2f}")
                        
                        st.caption("""
                        **Delta**: Directional exposure (-1 bearish, +1 bullish)
                    ...
                    st.caption(f"Greeks calculation: {e}")
                
                # Guidelines
                st.markdown("### Guidelines")
                
                if regime == "Compression":
                    st.info("✅ Good for premium selling strategies")
                elif regime == "Expansion":
                    st.warning("⚠️ Favor long options (buy volatility)")
                else:
                    st.info("ℹ️ Directional bias may help")
            
            except Exception as e:
                st.error(f"Risk profile: {e}")
